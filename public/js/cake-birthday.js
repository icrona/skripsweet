...
new Vue({
  ...
  data :{
    ...
    formErrors:{},
    formErrorsUpdate:{},
    newItem : {'name':'','description':'','category':'birthday','size':'','price':'','image':''},
    fillItem : {'name':'','description':'','category':'birthday','size':'','price':'','image':'','id':''},
    search : ''
  },
  computed: {
    isActived: function() {
  ...
  methods: {
    ...
    searchCake: function() {
      this.$http.get('/api/cake/birthday/search?search='+this.search).then((response) => {
        this.$set('cakes', response.data.data.data);
        this.$set('pagination', response.data.pagination);
      });
    },
    getVueItems: function(page) {
      this.$http.get('/api/cake/birthday?page='+page).then((response) => {
        this.$set('cakes', response.data.data.data);
        this.$set('pagination', response.data.pagination);
      });
    },
    createItem: function() {
      ...
      this.$http.post('/api/cake',input).then((response) => {
        ...
        this.newItem = {'name':'','description':'','category':'birthday','size':'','price':'','image':''};
        this.fileName='';
        $("#create-item-birthday").modal('hide');
    ...
    updateItem: function(id) {
      ...
      this.$http.put('/api/cake/'+id,input).then((response) => {
        this.changePage(this.pagination.current_page);
        this.newItem = {'name':'','description':'','category':'birthday','size':'','price':'','image':'','id':''};
