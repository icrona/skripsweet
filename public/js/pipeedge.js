...
new Vue({
  ...
  methods: {
    getVueItems: function() {
      this.$http.get('/api/manage/pipeedge').then((response) => {
    ...
    updateItem:function(id){
      ...
      console.log(input);
      this.$http.put('/api/manage/pipeedge'+id,input).then((response) => {
        this.getVueItems();
        $("#edit1").modal('hide')
        toastr.success('Pipe Edge - Bottom Updated Successfully.', 'Success Alert', {timeOut: 3000});
      }, (response) => {
